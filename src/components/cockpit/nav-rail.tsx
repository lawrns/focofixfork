...
export function NavRail({ onAction }: NavRailProps) {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <nav className="w-12 flex-shrink-0 bg-[#0d0d0f] border-r border-zinc-800/70 flex flex-col items-center py-2 overflow-y-auto">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <div key={item.label} className="w-full flex flex-col items-center">
                {item.divider && <div className="w-6 border-t border-zinc-800 my-1.5" />}
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive
                          ? 'bg-teal-500/15 text-teal-400 after:absolute after:left-0 after:top-1/2 after:-translate-y-1/2 after:h-4 after:w-0.5 after:rounded-full after:bg-teal-400'
        ...
        {/* Quick actions — visually grouped at bottom */}
        <div className="w-full flex flex-col items-center gap-0.5 pt-2 border-t border-zinc-800/80">
