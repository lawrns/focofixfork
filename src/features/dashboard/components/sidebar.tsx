...
const DashboardSidebar: React.FC<DashboardSidebarProps> = ({
      ...
      <div className="flex h-16 items-center justify-between px-4 border-b border-zinc-100 dark:border-zinc-800">
        ...
        {isMobile && (
          <Button
            ...
            className="h-8 w-8 p-0 rounded-md text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors duration-150"
          >
            <X className="h-4 w-4" />
          </Button>
        )}
  ...
  if (!isMobile) {
    return (
      <aside
        className={cn(
          'relative h-full w-64 flex flex-col',
  ...
  return (
    <Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}>
      <SheetContent
        side="left"
        className="w-full sm:w-80 p-0 bg-zinc-50/50 dark:bg-zinc-900 border-r border-zinc-100 dark:border-zinc-800 flex flex-col"
      >
        {sidebarContent}
      </SheetContent>
    </Sheet>
