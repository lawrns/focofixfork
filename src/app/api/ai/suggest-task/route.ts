...
import { AISuggestTaskSchema } from '@/lib/validation/schemas/ai-api.schema'
import { checkRateLimit } from '@/server/utils/rateLimit'
import { aiService } from '@/lib/services/openai'

/**
 * POST /api/ai/suggest-task - Suggest task using AI
 * Rate limited: 10 AI requests per minute
 */
export async function POST(request: NextRequest) {
  return wrapRoute(AISuggestTaskSchema, async ({ input, user, req, correlationId }) => {
