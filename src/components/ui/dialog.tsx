...
>(({ className, children, ...props }, ref) => {
  ...
  React.useEffect(() => {
    if (contentRef.current && props.onOpenAutoFocus) {
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg max-h-[90vh] overflow-y-auto focus:outline-none",
          // Mobile: full width with margin and safe area padding for iPhone home indicator
          // Desktop: constrained max-width
          isMobile
            ? "max-w-[calc(100vw-2rem)] pb-[calc(1.5rem+env(safe-area-inset-bottom))]"
            : "max-w-lg",
          className
        )}
        {...props}
      >
        ...
        <DialogPrimitive.Close
          className={cn(
            ...
            isMobile && "min-h-[44px] min-w-[44px] flex items-center justify-center -mr-2 -mt-2"
          )}
        >
