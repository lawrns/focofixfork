...
-- Database schema for FocoBot WhatsApp personal assistant
-- =====================================================

-- 1. Device Fingerprints
-- Stores device characteristics for security binding
CREATE TABLE IF NOT EXISTS whatsapp_device_fingerprints (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  phone TEXT NOT NULL,
  
  -- Device characteristics (hashed for privacy)
...
CREATE TABLE IF NOT EXISTS whatsapp_security_audit_log (
  ...
  -- Action
  action_type TEXT NOT NULL, -- 'message_received', 'command_executed', 'session_created', etc.
...
-- Track all commands executed by FocoBot
CREATE TABLE IF NOT EXISTS focobot_command_history (
  ...
  -- Command details
  command TEXT NOT NULL, -- 'list_tasks', 'create_task', 'complete_task', etc.
  command_input TEXT,
  ...
  -- LLM metadata
  llm_model TEXT, -- 'glm-4.7', 'claude-3-5-sonnet', etc.
  tokens_used INTEGER,
  processing_time_ms INTEGER,
...
-- Pending notification queue for users
CREATE TABLE IF NOT EXISTS focobot_scheduled_notifications (
  ...
  notification_type TEXT NOT NULL, -- 'daily_digest', 'task_reminder', 'deadline_alert'
  title TEXT NOT NULL,
...
-- FocoBot Command History: Users can view own commands
CREATE POLICY "Users can view own focobot commands"
  ON focobot_command_history FOR SELECT
  USING (auth.uid() = user_id);

-- FocoBot Notifications: Users can view own notifications
...
BEGIN
  INSERT INTO focobot_scheduled_notifications (
    ...
    p_user_id, p_phone, p_notification_type, p_title, message,
