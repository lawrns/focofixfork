...
function WeeklyStatusReport({ projectStatus, onExport, onShare, onShowSources }: { projectStatus: ProjectStatus[]; onExport: () => void; onShare: () => void; onShowSources: () => void }) {
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800">
          <p className="text-xs text-zinc-400 flex items-center gap-1">
            ...
            Generated by Critter AI based on recent activity.
            <Button variant="link" className="h-auto p-0 text-xs" onClick={onShowSources}>Show sources</Button>
          </p>
        </div>
      </CardContent>
    </Card>
...
export default function ReportsPage() {
  ...
  const handleExportReport = useCallback(() => {
    ...
    const csvContent = [
      'Project,Status,Progress,Tasks Completed,Total Tasks',
      ...projectStatus.map(p => `${p.name},${p.status},${p.progress}%,${p.tasksCompleted},${p.totalTasks}`)
    ].join('\n');
  ...
  const handleShowSources = useCallback(() => {
    toast.info('Report generated from task completion data, project progress metrics, and team activity logs');
  }, []);
