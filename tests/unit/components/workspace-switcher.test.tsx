...
describe('WorkspaceSwitcher Component', () => {
  ...
  it('opens dropdown and renders workspace items', async () => {
    ...
    expect(within(menu).getByRole('menuitem', { name: /engineering/i })).toBeInTheDocument()
    expect(within(menu).getByRole('menuitem', { name: /product/i })).toBeInTheDocument()
  })
  ...
  it('marks current workspace as active', async () => {
    ...
    const menu = await openMenu(user)
    const activeItem = within(menu).getByRole('menuitem', { name: /design team/i })
  ...
  it('switches workspace and navigates', async () => {
    ...
    const menu = await openMenu(user)
    await user.click(within(menu).getByRole('menuitem', { name: /engineering/i }))
  ...
  it('renders create workspace option and calls onCreateOpen', async () => {
    ...
    render(<WorkspaceSwitcher currentWorkspace="design-team" onCreateOpen={onCreateOpen} />)
    const menu = await openMenu(user)

    await user.click(within(menu).getByRole('menuitem', { name: /create workspace/i }))
  ...
  it('shows search input when there are at least 5 workspaces', async () => {
    ...
    await openMenu(user)
    expect(await screen.findByPlaceholderText(/search workspaces/i)).toBeInTheDocument()
  })
  ...
  it('calls workspace API with credentials include', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/workspaces', { credentials: 'include' })
    })
  })
  ...
  it('renders status region for announcements', () => {
    render(<WorkspaceSwitcher currentWorkspace="design-team" />)
