      ...
      <CardContent className="space-y-6">
        ...
        <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800">
          <p className="text-xs text-zinc-400 flex items-center gap-1">
            ...
            <Button variant="link" className="h-auto p-0 text-xs" onClick={onShowSources}>
              Show sources
            </Button>
          </p>
        </div>
...
export default function ReportsPageClient() {
  ...
  const handleExportReport = useCallback(() => {
    const csvContent = [
      ...
      ...projectStatus.map(
        (project) =>
          `${project.name},${project.status},${project.progress}%,${project.tasksCompleted},${project.totalTasks}`
      ),
    ].join('\n');
    ...
    anchor.href = url;
    anchor.download = `weekly-report-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    return (
      <PageShell>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[...Array(4)].map((_, index) => (
            <Card key={index} className="h-24 animate-pulse bg-zinc-100 dark:bg-zinc-800" />
          ))}
        </div>
