/**
 * OpenAI Project Management Service
 *
 * This service acts as the primary control interface for the entire project management system.
 * It enables OpenAI to parse natural language specifications and create/manage complete projects
...
Always return ONLY valid JSON with no additional text or explanations.`

    const userPrompt = `Parse this project specification and generate a complete project structure with milestones and tasks.
...
Return ONLY valid JSON, no explanations.`
    ...
    try {
      ...
      // Remove any potential non-JSON text before the opening brace
      const firstBrace = jsonText.indexOf('{')
      if (firstBrace > 0) {
        jsonText = jsonText.substring(firstBrace)
      }

      // Remove any potential non-JSON text after the closing brace
  ...
  static async updateProject(
    ...
    const systemPrompt = `You are a project management assistant. Parse update commands and return only the fields that should be changed as JSON.`

    const userPrompt = `Parse this update command for a project and return the changes to make.
...
Return ONLY valid JSON.`
  ...
  static async createMilestone(
    ...
    const systemPrompt = `You are a project management assistant. Create milestone specifications from descriptions. Return only JSON.`

    const userPrompt = `Create a milestone specification from this description.
...
Return ONLY valid JSON.`
  ...
  static async createTask(
    ...
    const systemPrompt = `You are a project management assistant. Create task specifications from descriptions. Return only JSON.`

    const userPrompt = `Create a task specification from this description.
